...
VALUES ('react_expert', 'hashed_password_1'),
...
-- Insert profiles
INSERT INTO user_profiles (id, background, goals, goal_vacancy)
VALUES (
        (
            SELECT id
            FROM users
            WHERE login = 'react_expert'
        ),
    ...
    (
        (
            SELECT id
            FROM users
            WHERE login = 'career_switcher_2024'
        ),
    ...
    (
        (
            SELECT id
            FROM users
            WHERE login = 'python_data_wizard'
        ),
